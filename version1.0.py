...
with mp_hands.Hands(False,1,min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:  # hands module
    while cap.isOpened(): # reading through each frame
        ...
        for i in range(0, 100, 1):
            ...
            cv2.line(image, (0, 50-i), (700, 50-i), white, 1)

        for j in range(0, 100, 1):
            cv2.line(image, (0, 425+j), (700, 425+j), white, 1)

        red_circle = cv2.circle(image, (red_x, red_y), 5, red, circle_thickness)
        green_circle = cv2.circle(image, (green_x, green_y), 5, green, circle_thickness)
        blue_circle = cv2.circle(image, (blue_x, blue_y), 5, blue, circle_thickness)
        white_circle = cv2.circle(image, (white_x, white_y), 5, white, circle_thickness)
        erase_button = cv2.putText(image, "ERASER", (450, 30), font, fontScale, black, 1)
        scale_1 = cv2.circle(image, (50, 455), 5, black, 10)
        scale_2 = cv2.circle(image, (150, 455), 5, black, 20)
        scale_3 = cv2.circle(image, (250, 455), 5, black, 30)
        save_button = cv2.putText(image, "SAVE", (350, 470), font, 1.5, black)
        clear_button = cv2.putText(image, "CLEAR", (500, 465), font, 1, red)
            ...
            for num, hand in enumerate(results.multi_hand_landmarks): # loop inside results
                ...
                if distance_im < 0.07:
                    ...
                    if coordinates_x > white_x-15 and coordinates_x < white_x + 15 and coordinates_y > white_y-10 and coordinates_y < white_y+10:
                        hand_color = white
                        print("white")
                    if coordinates_x > erase_button_X-15 and coordinates_x < erase_button_X+100 and coordinates_y > erase_button_y-10 and coordinates_y < erase_button_y+10:
                        hand_color = black
                        print("eraser")
                    if coordinates_x > save_button_x-5 and coordinates_x < save_button_x+100 and coordinates_y > save_button_y-10 and coordinates_y < save_button_y+10:
                ...
                if distance_it < 0.04:
                    ...
                    print(cv2.circle(image, coordinates, 5, hand_color, hand_thickness))
