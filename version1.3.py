...
# Define the center and radius of the 
center = (150, 150)
radius = 5
...
ret = cap.set(cv2.CAP_PROP_FRAME_HEIGHT,600)
width = 800
height = 600
new_coordinates = (0,0)
...
with mp_hands.Hands(False,1,min_detection_confidence=0.4, min_tracking_confidence=0.4) as hands:  # hands module
    while cap.isOpened(): # reading through each frame
        ...
        if results.multi_hand_landmarks: # check if there are some results
            for num, hand in enumerate(results.multi_hand_landmarks): # loop inside results
                ...
                if it_coordinates_x < 13 and it_coordinates_y < 13:
                    new_coordinates = (round((index_tip_x + thumb_tip_x) / 2), round((index_tip_y + thumb_tip_y) / 2))

                    my_coordinates = (round((index_tip.x + thumb_tip.x) / 2), round((index_tip.y + thumb_tip.y) / 2))

                    if past_coordinates == (0,0):
                        past_coordinates = new_coordinates

                    #cv2.circle(frame, mo_coordinates, 5, hand_color, hand_thickness)
                ...
                if distance_im < 0.07:
                    ...
                    if coordinates_x > redBrushTarget_x-0 and coordinates_x < redBrushTarget_x+30 and coordinates_y > redBrushTarget_y-0 and coordinates_y < redBrushTarget_y+50:
                        hand_color = red
                        if(hand_color == red):
                            cv2.circle(frame, (redBrushTarget_x+14, redBrushTarget_y+25), 28, black, 1)
                    if coordinates_x > greenBrushTarget_x-0 and coordinates_x < greenBrushTarget_x+30 and coordinates_y > greenBrushTarget_y-0 and coordinates_y < greenBrushTarget_y+50:
                        hand_color = green
                        if(hand_color == green):
                            cv2.circle(frame, (greenBrushTarget_x+14, greenBrushTarget_y+25), 28, black, 1)
                    if coordinates_x > blueBrushTarget_x-0 and coordinates_x < blueBrushTarget_x+30 and coordinates_y > blueBrushTarget_y-0 and coordinates_y < blueBrushTarget_y+50:
                        hand_color = blue
                        if(hand_color == blue):
                            cv2.circle(frame, (blueBrushTarget_x+14, blueBrushTarget_y+25), 28, black, 1)
                    if coordinates_x > whiteBrushTarget_x-0 and coordinates_x < whiteBrushTarget_x+30 and coordinates_y > whiteBrushTarget_y-0 and coordinates_y < whiteBrushTarget_y+50:
                        hand_color = white
                        if(hand_color == white):
                            cv2.circle(frame, (whiteBrushTarget_x+14, whiteBrushTarget_y+25), 28, black, 1)
                    if coordinates_x > save_button_x-0 and coordinates_x < save_button_x+200 and coordinates_y > save_button_y-20 and coordinates_y < save_button_y+0:
                    ...
                    if coordinates_x > scale_1_x-10 and coordinates_x < scale_1_x+20 and coordinates_y > scale_1_y-5 and coordinates_y > scale_1_y+100:
                        radius = 5
                        if radius == 5:
                            scale_1 = cv2.circle(frame, (scale_1_x, scale_1_y), 7, black, circle_thickness)
                    if coordinates_x > scale_2_x-10 and coordinates_x < scale_2_x+60 and coordinates_y > scale_2_y-5 and coordinates_y > scale_2_y+100:
                        radius = 10
                        if radius == 10:
                            scale_2 = cv2.circle(frame, (scale_2_x, scale_2_y), 7, black, circle_thickness)
                    if coordinates_x > scale_3_x-10 and coordinates_x < scale_3_x+90 and coordinates_y > scale_3_y-5 and coordinates_y > scale_3_y+100:
                        radius = 15
                        if radius == 15:
                            scale_3 = cv2.circle(frame, (scale_3_x, scale_3_y), 7, black, circle_thickness)
                    if coordinates_x > clear_button_x-0 and coordinates_x < clear_button_x+250 and coordinates_y > clear_button_y-20 and coordinates_y > clear_button_y+10:
                        canvas = np.zeros((600, 800, 3), np.uint8)
