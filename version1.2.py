...
radius = 10 # default radius for drawing circles
...
sent = False # letting us know if our drawing was sent to the selected email
...
# image paths
ngm_path = 'ngm.jpg'
ngm_image = cv2.imread(ngm_path) # path for ngm logo image
    ...
    while cap.isOpened(): # reading through each frame
        ...
        if results.multi_hand_landmarks: # check if there are some results
            for num, hand in enumerate(results.multi_hand_landmarks): # loop inside results
                ...
                # Get the landmarks of index/middle finger and thumb
                ...
                if it_coordinates_x < 13 and it_coordinates_y < 13:
                    ...
                    # cv2.circle(frame, my_coordinates, 5, hand_color, hand_thickness)
                ...
                if distance_im < 0.07:
                    ...
                    if coordinates_x > red_x-15 and coordinates_x < red_x + 45 and coordinates_y > red_y-5 and coordinates_y < red_y+30:
                        hand_color = red
                        if hand_color == red:
                            red_circle = cv2.circle(frame, (red_x, red_y), 7, red, circle_thickness)
                    if coordinates_x > green_x-20 and coordinates_x < green_x + 60 and coordinates_y > green_y-5 and coordinates_y < green_y+30:
                        hand_color = green
                        if hand_color == green:
                            green_circle = cv2.circle(frame, (green_x, green_y), 7, green, circle_thickness)
                    if coordinates_x > blue_x-20 and coordinates_x < blue_x + 60 and coordinates_y > blue_y-5 and coordinates_y < blue_y+30:
                        hand_color = blue
                        if hand_color == blue:
                            blue_circle = cv2.circle(frame, (blue_x, blue_y), 7, blue, circle_thickness)
                    if coordinates_x > white_x-1 and coordinates_x < white_x + 90 and coordinates_y > white_y-5 and coordinates_y < white_y+30:
                        hand_color = white
                        if hand_color == white:
                            white_circle = cv2.circle(frame, (white_x, white_y), 7, white, circle_thickness)
                            circle = cv2.circle(frame, (white_x, white_y), 23, black, 1)
                    if coordinates_x > erase_button_X-0 and coordinates_x < erase_button_X+300 and coordinates_y > erase_button_y-15 and coordinates_y < erase_button_y+5:
                        hand_color = black
                        if hand_color == black:
                            cv2.rectangle(frame, (440, 7), (570, 33), black, 1)
